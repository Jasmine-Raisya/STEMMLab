      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {team && (
          <View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            ...
            <Text style={[styles.rowSub, { color: colors.muted }]}>{t('common.teamId')}: {displayId}</Text>
            <Text style={[styles.rowSub, { color: colors.muted }]}>{team.representativeEmail}</Text>
          </View>
        )}

        {rows.map((row) => (
          <View key={row.label} style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            <View style={styles.row}>
              <View style={[styles.iconCircle, { backgroundColor: isDark ? 'rgba(207,196,107,0.24)' : stemmColors.greenSoft }]}>
                <Text style={styles.rowIcon}>{row.icon}</Text>
              </View>
              <View style={styles.rowInfo}>
                <Text style={[styles.rowLabel, { color: colors.text }]}>{row.label}</Text>
                <Text style={[styles.rowSub, { color: colors.muted }]}>{row.sub}</Text>
              </View>
